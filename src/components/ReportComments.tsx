...
function AdminBadge({ email }: { email: string }) {
    return (
        <Badge className="bg-blue-600 text-white flex items-center gap-1 shadow-md ring-2 ring-blue-300">
...
export default function ReportComments({ reportId, currentUserId }: { reportId: string; currentUserId: string }) {
    ...
    useEffect(() => {
        if (!reportId) return;
        setLoading(true);
        fetch(`/api/admin/monthly-reports/comments?report_id=${reportId}`)
            .then(res => res.json())
            .then(data => setComments(data.comments || []))
            .finally(() => setLoading(false));
    }, [reportId, refreshFlag]);
    ...
    const handlePostTopLevel = async () => {
        ...
        try {
            const res = await fetch('/api/admin/monthly-reports/comments', {
                method: 'POST',
...
function CommentThread({ comments, reportId, onReplySuccess, currentUserId, addReplyOptimistically, removeReplyById, replaceOptimisticReply }: {
    comments: Comment[];
    reportId: string;
    onReplySuccess: () => void;
                        ...
                        <Card className="p-3">
                            <div className="flex items-center gap-2 mb-1">
                                {comment.author_role === 'admin' ? (
                                    <AdminBadge email={comment.users?.email || 'Unknown admin'} />
