...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'auth.users') THEN
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reminder_logs') THEN
        ...
        CREATE INDEX "idx_reminder_logs_user_week" ON "reminder_logs" USING btree ("user_id","year","week_number");
    END IF;
END $$;